---
title: '13.1 Applications'
---
- 返回总讲义：[CHEM10712 slides 知识点整理与讲解](/uom-chemistry/chem10712/chem10712_slides_knowledge_notes_zh/)
- 知识点索引：[CHEM10712 知识点索引](/uom-chemistry/chem10712_knowledge_points/00-chem10712-知识点索引/)
- 上级章节：[13. Recombinant proteins 与 genome editing](/uom-chemistry/chem10712_knowledge_points/13-recombinant-proteins-与-genome-editing/)
- 导航：上一节：[12.3 DNA manipulation 概念](/uom-chemistry/chem10712_knowledge_points/12-03-dna-manipulation-概念/)；下一节：[13.2 Plasmids](/uom-chemistry/chem10712_knowledge_points/13-02-plasmids/)

## 详细内容

课件列出的应用：

- enzymes in washing powder。
- recombinant insulin。
- DNA arrays for cancer biomarkers。
- biosynthetic drug production，例如 artemisinin。
- biofuel production in engineered microorganisms。
- attenuated bacteria for vaccines。

<!-- BEGIN_DETAILED_EXPLANATION -->

## 深入讲解

recombinant DNA technology 的应用共同点是把一个 DNA sequence 放进新 host 或新 context，让细胞产生目标 molecule 或获得新功能。recombinant insulin 是表达治疗蛋白；detergent enzymes 是表达稳定工业酶；artemisinin pathway 是把多步代谢功能工程化到微生物中。

这些例子跨越 medicine、diagnostics、agriculture、energy 和 sustainable manufacturing，说明 molecular biology 技术能把 biological function 转换成生产平台。

## 原文小点逐条解释

- **原文小点：** enzymes in washing powder。
  - **讲解：** 这个小点是在列 recombinant DNA applications；要说明每个应用通过哪种 DNA/protein/pathway engineering 产生实际产品或功能。
- **原文小点：** recombinant insulin。
  - **讲解：** 这个小点是在列 recombinant DNA applications；要说明每个应用通过哪种 DNA/protein/pathway engineering 产生实际产品或功能。
- **原文小点：** DNA arrays for cancer biomarkers。
  - **讲解：** 这个小点是在列 recombinant DNA applications；要说明每个应用通过哪种 DNA/protein/pathway engineering 产生实际产品或功能。
- **原文小点：** biosynthetic drug production，例如 artemisinin。
  - **讲解：** 这个小点是在列 recombinant DNA applications；要说明每个应用通过哪种 DNA/protein/pathway engineering 产生实际产品或功能。
- **原文小点：** biofuel production in engineered microorganisms。
  - **讲解：** 这个小点是在列 recombinant DNA applications；要说明每个应用通过哪种 DNA/protein/pathway engineering 产生实际产品或功能。
- **原文小点：** attenuated bacteria for vaccines。
  - **讲解：** 这个小点是在列 recombinant DNA applications；要说明每个应用通过哪种 DNA/protein/pathway engineering 产生实际产品或功能。

## 做题和复习时怎么用

列应用时最好每个应用写“做了什么分子操作”和“产生什么功能/产品”，例如 express human insulin gene in microbes to produce therapeutic insulin.

## 常见误区

- 只列应用或工具名称，不说明 vector、host、selection marker、expression element 或 editing component 的具体功能。
- 把 recombinant protein expression 和 genome editing 混为一谈；前者主要表达目标产物，后者改变 genome sequence。

## 关联知识点

- [DNA manipulation](/uom-chemistry/chem10712_knowledge_points/12-03-dna-manipulation-概念/)
- [metabolic engineering](/uom-chemistry/chem10712_knowledge_points/14-01-metabolic-engineering-策略/)

<!-- END_DETAILED_EXPLANATION -->
