---
title: '7.1 Lipid 的定义'
---
- 返回总讲义：[CHEM10712 slides 知识点整理与讲解](/uom-chemistry/chem10712/chem10712_slides_knowledge_notes_zh/)
- 知识点索引：[CHEM10712 知识点索引](/uom-chemistry/chem10712_knowledge_points/00-chem10712-知识点索引/)
- 上级章节：[7. Lipids 与 membranes](/uom-chemistry/chem10712_knowledge_points/07-lipids-与-membranes/)
- 导航：上一节：[6.6 Glycosidic bonds 与 reducing sugars](/uom-chemistry/chem10712_knowledge_points/06-06-glycosidic-bonds-与-reducing-sugars/)；下一节：[7.2 Fatty acids](/uom-chemistry/chem10712_knowledge_points/07-02-fatty-acids/)

## 详细内容

Lipids 是 soluble in organic solvents but sparingly soluble in water 的 biomolecules。

包括：

- fatty acids。
- triacylglycerols。
- glycerophospholipids。
- sphingolipids。
- glycolipids。
- steroids。
- some vitamins/hormones。
- non-protein membrane components。

<!-- BEGIN_DETAILED_EXPLANATION -->

## 深入讲解

lipid 的定义是操作性定义：更溶于有机溶剂、较难溶于水。它们不一定有统一 polymer backbone，因此比 protein/nucleic acid 更像一组按物理性质归类的 molecules。

lipids 的核心功能来自 hydrophobicity 和 amphiphilicity：纯疏水部分适合储能；带 polar head 的 amphiphilic lipids 能自组装成 membranes；steroids 则以刚性环状骨架调节膜性质或作为 hormones。

## 原文小点逐条解释

- **原文小点：** fatty acids。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** triacylglycerols。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** glycerophospholipids。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** sphingolipids。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** glycolipids。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** steroids。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** some vitamins/hormones。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。
- **原文小点：** non-protein membrane components。
  - **讲解：** 这个小点属于 lipid classification；要联系 hydrophobicity/amphiphilicity 与储能、膜形成、信号或药物相关功能。

## 做题和复习时怎么用

如果问 lipid classes，列 fatty acids、triacylglycerols、phospholipids、sphingolipids、glycolipids、steroids，并各写一个功能。

## 常见误区

- 只背定义，不解释结构如何导致功能。
- 答题时缺少“因为...所以...”的因果链。

## 关联知识点

- [疏水效应](/uom-chemistry/chem10712_knowledge_points/02-01-为什么biochemistry-is-aqueous-chemistry/)
- [membrane/protein expression](/uom-chemistry/chem10712_knowledge_points/13-04-expression-hosts-比较/)

<!-- END_DETAILED_EXPLANATION -->
