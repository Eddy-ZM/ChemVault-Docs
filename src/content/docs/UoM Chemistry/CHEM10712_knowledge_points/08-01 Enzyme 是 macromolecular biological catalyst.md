---
title: '8.1 Enzyme 是 macromolecular biological catalyst'
---
- 返回总讲义：[CHEM10712 slides 知识点整理与讲解](/uom-chemistry/chem10712/chem10712_slides_knowledge_notes_zh/)
- 知识点索引：[CHEM10712 知识点索引](/uom-chemistry/chem10712_knowledge_points/00-chem10712-知识点索引/)
- 上级章节：[8. 酶：specificity、cofactors、allostery 与 kinetics](/uom-chemistry/chem10712_knowledge_points/08-酶specificitycofactorsallostery-与-kinetics/)
- 导航：上一节：[7.6 Membrane permeability](/uom-chemistry/chem10712_knowledge_points/07-06-membrane-permeability/)；下一节：[8.2 Active site interactions 与 stereospecificity](/uom-chemistry/chem10712_knowledge_points/08-02-active-site-interactions-与-stereospecificity/)

## 详细内容

与普通催化剂相比，酶通常有：

- higher reaction rate。
- mild conditions：aqueous solution、near-neutral pH、ambient temperature/pressure。
- high specificity：substrate/product stereospecificity、regiospecificity、limited side products。
- capacity for control：allosteric control、covalent modification、expression/degradation control。

酶反应基本写法：

```text
E + S -> E + P
```

但更真实模型包括 enzyme-substrate complex、intermediates、product dissociation 等步骤。

<!-- BEGIN_DETAILED_EXPLANATION -->

## 深入讲解

enzyme 是 catalyst，所以它不改变反应总体 ΔG 或 equilibrium position，而是降低 activation barrier，让反应更快达到平衡。酶的独特之处在于 active site 提供高度组织化的微环境：定位 substrate、排除/利用水、稳定 transition state，并通过 residues/cofactors 完成化学步骤。

“macromolecular” 很重要，因为 enzyme 的大结构提供 specificity 和 regulation。真正直接反应的可能只是几个 residues，但整个 protein fold 决定它们的空间排列、pKa、动态和底物入口。

## 原文小点逐条解释

- **原文小点：** higher reaction rate。
  - **讲解：** 这个小点是在说明 enzyme 作为 catalyst 的性质；要联系 rate enhancement、mild conditions、specificity 和 regulation。
- **原文小点：** mild conditions：aqueous solution、near-neutral pH、ambient temperature/pressure。
  - **讲解：** 这个小点是在说明 enzyme 作为 catalyst 的性质；要联系 rate enhancement、mild conditions、specificity 和 regulation。
- **原文小点：** high specificity：substrate/product stereospecificity、regiospecificity、limited side products。
  - **讲解：** 这个小点是在说明 enzyme 作为 catalyst 的性质；要联系 rate enhancement、mild conditions、specificity 和 regulation。
- **原文小点：** capacity for control：allosteric control、covalent modification、expression/degradation control。
  - **讲解：** 这个小点是在说明 enzyme 作为 catalyst 的性质；要联系 rate enhancement、mild conditions、specificity 和 regulation。

## 做题和复习时怎么用

问 enzyme vs non-enzyme catalyst 时写 rate enhancement、mild conditions、specificity、stereocontrol/regiocontrol、regulation。避免说 enzyme 改变 equilibrium。

## 常见误区

- 说酶改变反应平衡，而不是降低 activation barrier。
- 把 Km 简单等同于 Kd，忽略 kinetic model 条件。

## 关联知识点

- [active-site residues](/uom-chemistry/chem10712_knowledge_points/03-03-20-种氨基酸的功能分组/)
- [sitagliptin transaminase](/uom-chemistry/chem10712_knowledge_points/15-04-directed-evolution-case-sitagliptin-transaminase/)

<!-- END_DETAILED_EXPLANATION -->
